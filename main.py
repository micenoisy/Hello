...
async def get_script():
    ...
    except: return {"script": "NEVER REVEAL THE TRUTH. THE MOST DANGEROUS PERSON WATCHES EVERYTHING. THEY KNOW YOUR NEXT MOVE."}, {"red_words": ["never", "dangerous", "truth"], "yellow_words": ["everything", "move"]}

# [3] AUDIO & WHISPER SYNC
...
# [4] INSTA-PRO RENDER (Lower Position + Heavy Stroke)
def render(data, cfg, vp, dur, ts):
    print(f"🎬 VIDEO: Rendering Pro Style Reel...")
    ...
    for it in bd:
        ...
        sz = f"if(lt(t,{s}),0,if(lt(t,{s}+0.06),180,150))"
        
        # INSTA-PRO STACKED FILTERS:
        # Layer 1: Thick Outer Stroke (Black)
        draw.append(f"drawtext=text='{w}':{font}fontcolor=black@0.9:fontsize='{sz}+15':borderw=10:bordercolor=black:x=(w-text_w)/2:y={pos_y}:enable='between(t,{s},{e})'")
...
async def main():
    ...
    if os.path.exists("output/final.mp4"): print("✅ SUCCESS: Reel ready.")
